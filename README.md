...
이미지를 도안 형태의 픽셀 패턴으로 변환하고, 브라우저에서 직접 수정·저장할 수 있는 웹 기반 도안 편집기입니다.

> **현재 기준:** Frontend 편집기 MVP + FastAPI 이미지 변환 API가 구현되어 있으며, Django/MySQL/JWT 기반의 사용자·도안 저장 기능은 다음 단계에서 개발합니다.
...
`pixels[y][x]`에는 실제 HEX 값이 아니라 `palette`의 색상 `id`가 저장됩니다.
...
Canvas는 직접 데이터를 수정하지 않고 Renderer와 Layer를 통해 화면을 그립니다.
...
Grid를 각각의 Pixel 처리 안에서 반복해서 그리지 않고, **모든 Pixel을 먼저 그린 뒤 Grid를 한 번씩 렌더링**하도록 구성했습니다.
...
- 향후 Backend에서도 권한 검사를 수행하여 Frontend의 View/Edit 제한을 보완
